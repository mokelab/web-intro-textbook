...
//image[ractovelogo][Ractive.jsのロゴ]{
//}
...
これを実行すると、@<img>{intro}のように@<code>{script}内のHTMLが表示されます。

//image[intro][Ractive.jsを用いてテンプレートを表示した]{
//}
...
変更が完了したらブラウザで実行させてみましょう。@<img>{var}のように@<code>{{{name\}\}}の部分が@<code>{data}で指定した値に置き換えられて表示されています。ここでの変数@<code>{name}はRactiveオブジェクトが保持している変数であり、@<code>{var}で作成した変数とは異なる点に注意しましょう。

//image[var][テンプレートで変数を使用した例]{
//}
...
これをブラウザで実行すると、@<img>{for1}のように表示されます。

//image[for1][テンプレートで繰り返し処理]{
//}
...
この場合、テンプレートでは@<list>{for4}のようにオブジェクトのキーを@<code>{{{\}\}}で指定することで、@<img>{for2}のように値を取り出すことができます。

//list[for4][オブジェクトの配列を表示する]{
<script id="template" type="text/ractive">
  <h1>ユーザー一覧</h1>
  {{#users}}
  <p>{{name}}({{age}}才)</p>
...
もし変数ageが18の場合、@<img>{if1}のようにビールは表示されません。

//image[if1][変数ageが18の場合]{
//}

変数ageが21の場合、@<img>{if2}のようにビールは表示されます。

//image[if2][変数ageが21の場合]{
//}
...
これを実行し、入力フォームに文字列を入力すると、@<img>{twoway}のように自動的に変数nameの値が変化することがわかります。

//image[twoway][フォームに入力すると、即座に見出しも変化する]{
//}
...
これをブラウザで実行し、入力欄に名前を入力した後ボタンをクリックすると、@<img>{event}のようにアラートが表示されます。イベント処理中の@<code>{r.get('name')}は、Ractiveオブジェクトに保持している変数の値を教えてもらう処理です。

//image[event][ボタンをクリックするとイベントが実行された]{
//}
...
これを実行すると@<img>{countapp1}のような画面が表示されます。

//image[countapp1][実行結果]{
//}

ボタンをクリックすると@<img>{countapp2}のようにカウントが1つずつ増えていきます。

//image[countapp2][カウントが増えている]{
//}
